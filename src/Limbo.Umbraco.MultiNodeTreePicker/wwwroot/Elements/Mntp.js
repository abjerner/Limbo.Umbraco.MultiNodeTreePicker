 ...
 * @element limbo-mntp
 */
export class LimboMntpElement extends UmbFormControlMixin(UmbLitElement, undefined) {
	...
	focus() {
		return this.shadowRoot?.querySelector(INNER_TAG)?.focus();
	}
	...
	render() {
		...
		// NOTE: "value" must be bound before "config", as the built-in element inspects the current value when its
		// config is set (to detect items of an unsupported entity type).
		return html`
...
customElements.define("limbo-mntp", LimboMntpElement);

export default LimboMntpElement;
